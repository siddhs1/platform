...
- [ ] `[APPROVAL]` **Push commits to the git remote.** Claude commits locally but never runs `git push` without you saying so. Local `main` is currently ahead of `origin/main` (unpushed). Before the first push, review `.github/workflows/ci.yml` + `lighthouserc.cjs` (see §5).
...
## 5. CI / GitHub Actions
- [ ] `[APPROVAL]` Push the repo to GitHub (see §0).
