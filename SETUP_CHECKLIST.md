# Owner Setup Checklist â€” things Claude can't do for you
...
**Status legend**
- `[SET]` â€” already configured and working
- `[DEV-STUB]` â€” works in local dev via a degrade/bypass; real credentials needed before production
- `[NEEDED]` â€” required for the related feature to function at all
- `[LATER]` â€” not needed yet; belongs to a later phase (noted per item)
- `[APPROVAL]` â€” Claude can do it, but only with your explicit go-ahead
...
## 0. Approvals â€” actions Claude will not take on its own
...
- [x] `[SET]` **`DATABASE_URL`** (pooled, runtime) and **`DIRECT_URL`** (direct, migrations). Working â€” migrations `0000`â€“`0004` are applied and the demo data is seeded.
...
## 2. Console authentication â€” Clerk
...
- [ ] `[LATER]` **Clerk Organizations** (one org per tenant; `client_admin` / `client_staff` roles) â€” required for the tenant (client) console in Step 3.

## 3. Billing â€” Stripe (agency's own retainers)
...
## 4. Notifications â€” new-lead alerts
Each channel degrades independently (unset keys = that channel is skipped, never a crash).
- [ ] `[NEEDED]` **Email via Resend** â€” set **`RESEND_API_KEY`** and **`RESEND_FROM`** (verify a sending domain in Resend).
- [ ] `[NEEDED]` **SMS via Twilio** â€” set **`TWILIO_ACCOUNT_SID`**, **`TWILIO_AUTH_TOKEN`**, **`TWILIO_FROM_NUMBER`** (buy a number). The same Twilio number backs the call-tracking voice webhook.
...
- [ ] `[NEEDED]` In the GitHub repo settings, add **Actions secrets `DATABASE_URL` and `DIRECT_URL`** â€” the Lighthouse CI job needs them (the build itself passes without DB secrets thanks to the lazy client). Confirm the workflow is green on the first push.

## 6. Custom domains â€” Cloudflare for SaaS (Step 4)
